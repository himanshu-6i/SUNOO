      ...
      {isExpanded && (
        <div className="fixed inset-0 z-[100] bg-gradient-to-b from-zinc-900 to-black flex flex-col px-6 pt-12 pb-8 md:hidden animate-in slide-in-from-bottom-full duration-300">
          <div className="flex items-center justify-between mb-8">
            ...
            <button onClick={(e) => { e.stopPropagation(); onToggleFullscreen?.(); }} className="text-white focus:outline-none">
              <Maximize2 className="w-5 h-5" />
            </button>
          </div>

          <div className="flex-1 flex items-center justify-center mb-8 w-full max-w-sm mx-auto">
            <img src={currentTrack.coverUrl} className="w-full aspect-square object-cover rounded-xl shadow-2xl" />
          </div>
          ...
          <div className="flex items-center justify-between mb-8 px-2">
            ...
            <button onClick={(e) => { e.stopPropagation(); onTogglePlay(); }} className="w-16 h-16 bg-white rounded-full flex items-center justify-center focus:outline-none text-black">
              {isPlaying ? (
          ...
          <div className="flex justify-between items-center px-2 pb-4">
             <div className="flex gap-6 items-center">
               <button onClick={(e) => { e.stopPropagation(); onToggleEQ?.(); }} className="text-zinc-400 hover:text-white transition-colors" title="EQ"><SlidersHorizontal className="w-6 h-6" /></button>
               <button onClick={(e) => {
                 e.stopPropagation();
                 if (isMuted) {
                   setIsMuted(false);
        ...
        <div className="flex flex-col items-center max-w-[600px] w-1/2 md:w-2/4 gap-1 md:gap-2">
          <div className="flex items-center justify-end md:justify-center gap-4 md:gap-6 w-full">
            ...
            <button 
              ...
              className="w-10 h-10 md:w-11 md:h-11 shrink-0 bg-transparent md:bg-gradient-to-r from-violet-600 to-fuchsia-600 md:rounded-full flex items-center justify-center hover:scale-105 transition-all md:shadow-[0_0_15px_rgba(168,85,247,0.5)]"
            >
              {isPlaying ? (
                <Pause className="w-6 h-6 md:w-5 md:h-5 text-white fill-current" />
              ) : (
                <Play className="w-6 h-6 md:w-5 md:h-5 text-white fill-current ml-0.5 md:ml-1" />
              )}
            </button>
          ...
          {/* Progress Bar */}
          <div className="hidden md:flex items-center gap-4 w-full px-4 text-[11px] text-zinc-400 font-mono tracking-wider font-medium" onClick={(e) => e.stopPropagation()}>
            <span className="w-8 text-right">{currentTime}</span>
            <div className="flex-1 relative flex items-center h-4 group w-full">
